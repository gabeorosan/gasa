...
var n;
var lyricsFile;
var lyrics, currentSentence, words, wordLines, title
...
// Function to save the current song title
function saveSong() {
  /*
  let savedSongs = JSON.parse(localStorage.getItem('savedSongs')) || {};
  if (!(title in savedSongs)) {
    savedSongs[title] = 1;
...
document.getElementById("saveSongButton").addEventListener("click", function() {
  var icon = document.getElementById("saveSongButton");
  icon.classList.add("clicked"); // Make it white immediately
  saveSong();
  // Remove clicked class after animation duration
  setTimeout(function() {
    icon.classList.remove("clicked");
  }, 1200); // 1s duration of the animation
});
